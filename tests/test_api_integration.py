...
class TestAPIIntegration:
    ...
    def sample_excel_file(self, temp_dir):
        ...
        # Headers
        headers = [
            "YAZAKI PN", "BX726_BEV_JOB1+90_YMOK", "BX726_BEV_JOB1+90_YRL",
            "BX726_MCA_JOB1+90_YMOK", "Item Description", "Supplier Name",
            "PSW", "Approved Date", "FAR Status"
        ]
        ...
        # Sample data
        data = [
            ["7009-6933", "X", "", "D", "Wire Harness", "Yazaki Corp", "OK", "2024-01-15", "OK"],
            ["7116-4101-02", "D", "X", "", "Connector", "Molex Inc", "NG", "2024-02-20", "NG"],
            ["ABC-123", "", "0", "X", "Terminal", "TE Connectivity", "", "2024-03-25", "Pending"]
        ]
    ...
    async def test_complete_workflow(self, client, sample_excel_file):
        ...
        with open(sample_excel_file, "rb") as f:
            files = {"file": ("test_workbook.xlsx", f, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")}
        ...
        expected_artifacts = [
            "masterbom_clean.csv", "masterbom_clean.parquet",
            "plant_item_status.csv", "plant_item_status.parquet",
            "fact_parts.csv", "fact_parts.parquet",
            "status_clean.csv", "status_clean.parquet",
            "dim_dates.csv", "dim_dates.parquet",
            "etl.sqlite"
        ]
    ...
    async def test_large_file_handling(self, client, temp_dir):
        ...
        with open(file_path, "rb") as f:
            files = {"file": ("large_workbook.xlsx", f, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")}
            upload_response = await client.post("/api/upload", files=files)
...
class TestDataValidation:
    ...
    def test_status_classification_rules(self):
        ...
        for raw_status, expected_class in test_cases:
            ...
            assert result == expected_class, f"Failed for {raw_status}: expected {expected_class}, got {result}"
