...
class TestMasterBOMProcessor:
    ...
    def sample_masterbom_data(self):
        ...
        return pd.DataFrame({
            "YAZAKI PN": ["7009-6933", "7116-4101-02", "ABC-123", "7009-6933"],  # Duplicate
            "BX726_BEV_JOB1+90_YMOK": ["X", "D", "", "X"],
            "BX726_BEV_JOB1+90_YRL": ["", "X", "0", ""],
            "BX726_MCA_JOB1+90_YMOK": ["D", "", "X", "D"],
            "Item Description": ["Wire Harness", "Connector", "Terminal", "Wire Harness"],
            "Supplier Name": ["  yazaki corp  ", "MOLEX INC", "te connectivity", "  yazaki corp  "],
            "PSW": ["OK", "NG", "", "OK"],
    ...
    def test_clean_column_names(self, sample_masterbom_data, mock_logger):
        ...
        sample_masterbom_data.columns = [
            ...
            "PSW", "PSW Type", "Approved Date", "PSW Date OK", "FAR Status",
    ...
    def test_identify_columns(self, sample_masterbom_data, mock_logger):
        ...
        expected_projects = ["BX726_BEV_JOB1+90_YMOK", "BX726_BEV_JOB1+90_YRL", "BX726_MCA_JOB1+90_YMOK"]
    ...
    def test_create_fact_parts(self, sample_masterbom_data, mock_logger):
        ...
        # Check structure
        expected_columns = [
            "part_id_std", "part_id_raw", "Item Description", "Supplier Name",
            "PSW", "PSW Type", "FAR Status", "Handling Manual"
        ]
        for col in expected_columns:
            assert col in result.columns
...
class TestMasterBOMEdgeCases:
    ...
    def test_very_large_dataset(self, mock_logger):
        ...
        data = {
            "YAZAKI PN": [f"PART-{i:04d}" for i in range(n_parts)]
        }
