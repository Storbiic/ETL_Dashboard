...
def sample_excel_data():
    ...
    return {
        "masterbom": pd.DataFrame({
            ...
            "Supplier Name": ["Yazaki Corp", "Molex Inc", "TE Connectivity", "Littelfuse"],
            "PSW": ["OK", "NG", "", "OK"],
            "PSW Type": ["Type A", "Type B", "Type C", "Type A"],
            "Approved Date": ["2024-01-15", "2024-02-20", "2024-03-25", "2024-01-30"],
...
def mock_etl_logger():
    ...
    logger.warning.side_effect = lambda msg, **kwargs: mock_log("warning", msg, **kwargs)
...
def sample_cleaned_data():
    ...
    return {
        "masterbom_clean": pd.DataFrame({
            ...
            "PSW": ["OK", "NG", ""],
            "Approved Date_date": pd.to_datetime(["2024-01-15", "2024-02-20", "2024-03-25"]).date,
            "Approved Date_year": [2024, 2024, 2024],
            "Approved Date_month": [1, 2, 3]
        }),
        "plant_item_status": pd.DataFrame({
            "part_id_std": ["7009-6933", "7009-6933", "7116-4101-02", "7116-4101-02"],
...
def generate_large_masterbom(n_parts=1000, n_projects=10):
    ...
    data = {
        "YAZAKI PN": [f"PART-{i:06d}" for i in range(n_parts)]
    }
    
    # Add project columns
    for j in range(n_projects):
        project_name = f"PROJECT_{j:02d}_PLANT"
        data[project_name] = np.random.choice(["X", "D", "", "0"], n_parts, p=[0.4, 0.2, 0.3, 0.1])
    
    data.update({
        "Item Description": [f"Component {i}" for i in range(n_parts)],
        "Supplier Name": np.random.choice(["Supplier A", "Supplier B", "Supplier C"], n_parts),
        "PSW": np.random.choice(["OK", "NG", ""], n_parts, p=[0.6, 0.2, 0.2]),
        "Approved Date": pd.date_range("2023-01-01", periods=n_parts, freq="D").strftime("%Y-%m-%d"),
        "FAR Status": np.random.choice(["OK", "NG", "Pending"], n_parts, p=[0.5, 0.2, 0.3])
    })
...
def generate_problematic_data():
    ...
    return pd.DataFrame({
        ...
        "Item Description": ["", "Valid Description", None, "Spéciàl Chars", "Normal"],
        "Supplier Name": [None, "", "UPPERCASE SUPPLIER", "  spaced supplier  ", "Normal Supplier"],
