...
async def profile_sheet(
    ...
    try:
        ...
        logger.info("Starting sheet profiling", 
        ...
        try:
            ...
            logger.info("Sheet profiling completed", 
                       file_id=file_id,
                       sheet=sheet,
        ...
        logger.error("Error during sheet profiling", 
                    file_id=file_id, sheet=sheet, error=str(e))
        
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error during profiling: {str(e)}"
        )
...
async def compare_sheets(
    ...
    try:
        ...
        logger.info("Starting sheet comparison", 
                   file_id=file_id, sheet1=sheet1, sheet2=sheet2)
        ...
        try:
            ...
            if len(comparison["compatibility"]["common_columns"]) < 3:
                recommendations.append("Sheets have very few common columns - verify sheet selection")
            ...
            logger.info("Sheet comparison completed", 
                       file_id=file_id,
        ...
        logger.error("Error during sheet comparison", 
                    file_id=file_id, sheet1=sheet1, sheet2=sheet2, error=str(e))
        
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error during comparison: {str(e)}"
        )
