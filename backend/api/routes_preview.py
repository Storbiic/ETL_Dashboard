...
async def preview_sheet(
    ...
    n: int = Query(default=10, ge=1, le=100, description="Number of rows for head/tail")
):
    """
    ...
    try:
        ...
        logger.info("Starting sheet preview", 
                   file_id=file_id, sheet=sheet, preview_rows=n)
        ...
        try:
            ...
            logger.info("Sheet preview completed", 
                       file_id=file_id,
                       sheet=sheet,
        ...
        logger.error("Error during sheet preview", 
                    file_id=file_id, sheet=sheet, error=str(e))
        
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error during preview: {str(e)}"
        )
...
async def list_sheets(file_id: str):
    ...
    try:
        ...
        try:
            ...
            logger.info("Listed sheets", 
                       file_id=file_id, sheet_count=len(sheets_info))
        ...
        logger.error("Error listing sheets", 
                    file_id=file_id, error=str(e))
        
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error listing sheets: {str(e)}"
        )
    ...
    try:
        ...
        try:
            ...
            if sheet not in available_sheets:
                raise HTTPException(
                    status_code=400,
                    detail=f"Sheet '{sheet}' not found"
                )
            ...
            logger.info("Retrieved column info", 
                       file_id=file_id, sheet=sheet, column_count=len(columns_info))
        ...
        logger.error("Error getting column info", 
                    file_id=file_id, sheet=sheet, error=str(e))
        
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error getting columns: {str(e)}"
        )
