...
def _auto_detect_sheets(sheet_names: list[str]) -> dict:
    ...
    detected["suggestions"] = {
        "masterbom_candidates": sorted(masterbom_candidates, key=lambda x: x[1], reverse=True),
        "status_candidates": sorted(status_candidates, key=lambda x: x[1], reverse=True)
    }
