...
class PowerBIIntegration:
    ...
    def copy_files_to_pipeline(self, artifacts: List[Dict]) -> List[str]:
        ...
        try:
            self.logger.info("Copying files to pipeline output folder", 
            ...
            for artifact in artifacts:
                ...
                self.logger.info(f"Copied file: {source_path.name}", 
                               size_bytes=artifact.get('size_bytes', 0))
            
            self.logger.info("File copy completed", 
    ...
    def create_powerbi_template(self, dataframes: Dict[str, pd.DataFrame]) -> Optional[str]:
        """
...
in
    ...
    def _generate_column_types(self, df: pd.DataFrame) -> str:
        ...
        return f"Table.TransformColumnTypes(PromoteHeaders,{{{', '.join(column_types)}}})"
    ...
    def open_powerbi_template(self, template_path: str) -> bool:
        ...
        try:
            ...
            self.logger.info("PowerBI template opened", 
