...
class StatusProcessor:
    ...
    def _remove_empty_rows(self):
        ...
        mask = self.df.apply(
            lambda row: row.notna().any() and
                       any(str(val).strip() != '' for val in row if pd.notna(val)),
            axis=1
        )
    ...
    def get_project_summary(self) -> Dict[str, any]:
        ...
        try:
            ...
            if 'Total_Part_Numbers' in self.df.columns:
            ...
            if 'PSW_Available' in self.df.columns:
            ...
            if 'Drawing_Available' in self.df.columns:
            ...
            if 'OEM' in self.df.columns and 'Project' in self.df.columns:
