    ...
    def _extract_project_completion_by_plant(self) -> pd.DataFrame:
        ...
        try:
            ...
            for col in columns:
                ...
                if any(keyword in col_lower for keyword in ['milestone', 'ppap', 'date', 'deadline']):
                    milestone_cols.append(col)

                # Completion/percentage columns
                if any(keyword in col_lower for keyword in ['%', 'percent', 'complete', 'available', 'status']):
                    completion_cols.append(col)

            self.logger.info("Identified completion analysis columns",
            ...
            for idx, row in self.df.iterrows():
                try:
                    ...
                    project_name = str(row.get(project_col, '')).strip() if project_col else f"Project_{idx}"
                    oem_name = str(row.get(oem_col, '')).strip() if oem_col else 'Unknown'
                    ...
                    if not plants:
                        plants = [{'plant_id': project_name, 'plant_name': project_name}]
                    ...
                    for plant_info in plants:
                        ...
                        for col in completion_cols:
                            ...
                            if pd.notna(value):
                                ...
                                if 'psw' in col_lower:
                                    ...
                                    completion_record['overall_completion_pct'] = completion_pct
                        ...
                        for col in milestone_cols:
                            ...
                            if pd.notna(value):
                                try:
                                    milestone_date = pd.to_datetime(value, errors='coerce')
                                    if pd.notna(milestone_date):
                                        completion_record['milestone_date'] = milestone_date.date()
                        ...
                        for col in columns:
                            if 'total' in str(col).lower() and 'part' in str(col).lower():
                                try:
                                    value = row[col]
                                    # If value is a Series, take the first non-null value
                                    if hasattr(value, 'iloc'):
                                ...
                                if pd.notna(value):
                                    try:
                                        completion_record['total_parts'] = int(float(str(value)))
                                        break
                                    except:
                                        continue

                        # Determine overall completion status
                        completion_record['completion_status'] = self._determine_completion_status(completion_record)

                        completion_records.append(completion_record)

                except Exception as e:
                    self.logger.warning(f"Failed to process completion data for row {idx}: {e}")
                    continue
            ...
            if completion_records:
                ...
                completion_df = completion_df.drop_duplicates(subset=['project_name', 'plant_id'], keep='first')

                self.logger.info("Project completion extraction complete",
