...
class ExcelReader:
    ...
    def get_sheet_names(self) -> List[str]:
        ...
        if self._sheet_names is None:
            try:
                ...
                logger.info(f"Found {len(self._sheet_names)} sheets", 
                           file=str(self.file_path), sheets=self._sheet_names)
    ...
    def detect_project_columns(self, df: pd.DataFrame, 
                              id_col: str = "YAZAKI PN") -> Tuple[List[str], int, int]:
        """
        ...
        try:
            ...
            if desc_col_idx is None:
                logger.warning("Description column not found, assuming all columns after ID are projects")
            ...
            logger.info(f"Detected {len(project_columns)} project columns",
                       start_idx=project_start, end_idx=project_end,
                       columns=project_columns[:5])  # Log first 5
