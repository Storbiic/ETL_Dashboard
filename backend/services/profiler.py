...
class DataProfiler:
    ...
    def _infer_dtype(self, series: pd.Series) -> str:
        ...
        try:
            ...
            # Check if integers
            if all(str(val).replace('.', '').replace('-', '').isdigit() 
                   for val in non_null if str(val).strip()):
                return "integer"
            else:
                return "numeric"
    ...
    def count_duplicate_rows(self) -> int:
        ...
        try:
            ...
            non_null_cols = [col for col in self.df.columns 
    ...
    def profile_sheet(self) -> ProfileResponse:
        ...
        logger.info(f"Profiling sheet '{self.sheet_name}'", 
        ...
        logger.info(f"Profiling complete for '{self.sheet_name}'",
