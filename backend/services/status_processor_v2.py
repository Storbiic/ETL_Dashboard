...
class StatusProcessorV2:
    ...
    def __init__(self, df: pd.DataFrame, logger: ETLLogger):
        ...
        self.required_columns = [
            'plant_id', 'oem', 'sqe', 'milestone_date', 'total_parts',
            'psw_available', 'psw_completion_pct',
            'drawing_available', 'drawing_completion_pct',
            'imds_total', 'imds_completion_pct',
            'm2_parts', 'm2_parts_psw_ok', 'ppap_completion_pct',
            'overall_completion_pct', 'completion_status', 'bom_file_date'
        ]
    ...
    def process(self) -> Dict[str, pd.DataFrame]:
        ...
        """
        try:
            self.logger.info("Starting Status sheet processing (V2)",
            ...
            self.logger.info("Status sheet processing complete (V2)",
            ...
            self.logger.error(f"Error in status processing (V2): {str(e)}",
    ...
    def _clean_and_prepare_data(self):
        ...
        for col in self.df.columns:
            if self.df[col].isna().all() or (self.df[col].astype(str).str.strip() == '').all():
        ...
        if fully_empty_rows.any():
            ...
            if first_empty_idx > 0:  # Keep at least some data
                ...
                self.logger.info(f"Truncated at first fully empty row: {first_empty_idx}")
        
        # Drop rows that are fully NaN
        self.df = self.df.dropna(how='all')
        
        self.logger.info("Data cleaning complete", 
                        final_rows=len(self.df), final_cols=len(self.df.columns))
    ...
    def _normalize_headers_and_map_schema(self):
        ...
        for original_col, normalized_header in normalized_headers.items():
            ...
            for source_pattern, target_name in self.column_mapping.items():
                if target_name not in used_targets and self._header_matches(normalized_header, source_pattern):
        ...
        self.logger.info("Header normalization complete",
    ...
    def _apply_type_coercion_and_derived_fields(self):
        ...
        count_columns = ['total_parts', 'psw_available', 'drawing_available',
                        'imds_total', 'm2_parts', 'm2_parts_psw_ok']
        for col in count_columns:
            if col in self.df.columns:
                # Check if column exists and is not duplicated
                col_data = self.df[col]
                if hasattr(col_data, 'iloc'):  # It's a Series
                    self.df[col] = pd.to_numeric(col_data, errors='coerce').astype('Int64')
                else:
                    # Handle potential duplicate columns by taking first
                    self.df[col] = pd.to_numeric(self.df[col].iloc[:, 0] if len(self.df[col].shape) > 1 else self.df[col], errors='coerce').astype('Int64')

        # Percentage columns
        pct_columns = ['psw_completion_pct', 'drawing_completion_pct', 'imds_completion_pct']
        for col in pct_columns:
            if col in self.df.columns:
                ...
                if hasattr(col_data, 'iloc'):  # It's a Series
                    ...
                    self.df[col] = self._parse_percentage_column(self.df[col].iloc[:, 0] if len(self.df[col].shape) > 1 else self.df[col])
    ...
    def _create_derived_fields(self):
        ...
        pct_cols = ['psw_completion_pct', 'drawing_completion_pct', 
                   'imds_completion_pct', 'ppap_completion_pct']
        
        available_pct_cols = [col for col in pct_cols if col in self.df.columns]
        
        if available_pct_cols:
            self.df['overall_completion_pct'] = self.df[available_pct_cols].mean(axis=1, skipna=True)
        else:
        ...
        for col in self.required_columns:
            if col not in self.df.columns:
                ...
                if col.endswith('_pct'):
                    ...
                    self.df[col] = pd.NaT
                elif col in ['total_parts', 'psw_available', 'drawing_available', 
                           'imds_total', 'm2_parts', 'm2_parts_psw_ok']:
                    self.df[col] = pd.NA
                else:
                    self.df[col] = None
